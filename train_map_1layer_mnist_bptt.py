...
class SharedMaskLinear(nn.Module):
    def __init__(
        self,
        in_features: int,
        out_features: int,
        *,
        bias: bool = True,
        shared_w_init: float = 0.03,
        shared_w_min: float = -2.0,
        shared_w_max: float = 2.0,
        mask_logit_init: float = 0.0,
        mask_hard_eval: int = 1,
    ):
        super().__init__()
        self.in_features = int(in_features)
        self.out_features = int(out_features)
        self.shared_w_min = float(min(shared_w_min, shared_w_max))
        self.shared_w_max = float(max(shared_w_min, shared_w_max))
        self.mask_hard_eval = int(mask_hard_eval)

        self.shared_w = nn.Parameter(torch.empty((self.out_features,), dtype=torch.float32))
        with torch.no_grad():
            self.shared_w.normal_(mean=0.0, std=float(max(1.0e-5, shared_w_init)))

        self.mask_logits = nn.Parameter(
            torch.full((self.out_features, self.in_features), float(mask_logit_init), dtype=torch.float32)
        )
        if bool(bias):
        ...
        self.register_buffer("_mask_beta", torch.tensor(8.0, dtype=torch.float32))

    def set_mask_beta(self, beta: float) -> None:
        with torch.no_grad():
            self._mask_beta.fill_(float(max(1.0e-4, beta)))

    def _mask_prob(self) -> torch.Tensor:
        b = float(self._mask_beta.item())
        return torch.sigmoid(b * self.mask_logits)

    def mask_stats(self) -> tuple[torch.Tensor, torch.Tensor]:
        p = self._mask_prob()
        h = (p >= 0.5).to(p.dtype)
        return p.mean(), h.mean()

    def _effective_weight(self) -> torch.Tensor:
        p = self._mask_prob()
        h = (p >= 0.5).to(p.dtype)
        if self.training:
            m = p + (h - p).detach()  # hard forward, soft backward
        else:
            m = h if int(self.mask_hard_eval) == 1 else p
        sw = torch.clamp(self.shared_w, min=self.shared_w_min, max=self.shared_w_max).unsqueeze(1)
        return sw * m
